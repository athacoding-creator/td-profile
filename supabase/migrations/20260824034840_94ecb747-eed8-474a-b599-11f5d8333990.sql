ALTER TABLE public.event_position_pricing ADD COLUMN IF NOT EXISTS description text;

UPDATE public.events SET event_type = 'olahraga' WHERE event_type = 'futsal';
UPDATE public.events SET event_type = 'kelas-kajian' WHERE event_type = 'mini-soccer';
...
BEGIN
  ...
  IF ev.event_type IN ('futsal', 'mini-soccer', 'olahraga', 'kelas-kajian') AND ev.registration_type = 'paid' THEN
    IF NEW.position IS NULL OR NEW.position = '' THEN
      RAISE EXCEPTION 'Pilih posisi/kelas terlebih dahulu';
    END IF;
    ...
    IF pos_price IS NULL THEN
      RAISE EXCEPTION 'Harga untuk pilihan % belum diatur admin', NEW.position;
    END IF;
    IF NEW.amount_paid IS NULL OR abs(NEW.amount_paid - pos_price) > 1 THEN
      RAISE EXCEPTION 'Nominal harus sesuai harga pilihan (%). Diterima: %', pos_price, NEW.amount_paid;
    END IF;
    RETURN NEW;
  END IF;
