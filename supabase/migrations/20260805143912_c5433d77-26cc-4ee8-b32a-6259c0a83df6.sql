...
BEGIN
  ...
  WHERE event_id = evid AND user_id = auth.uid()
  LIMIT 1;

  IF ev.registration_type <> 'free' THEN
    IF reg.id IS NULL THEN
      RAISE EXCEPTION 'Kamu belum menyelesaikan pendaftaran event ini';
    END IF;
  ...
  IF ev.registration_type = 'free' AND reg.id IS NULL THEN
    INSERT INTO public.registrations(
      event_id, user_id, registered_by, payment_status, amount_paid, attendance_mode
    ) VALUES (
      evid, auth.uid(), auth.uid(), 'none', 0, 'offline'
    )
